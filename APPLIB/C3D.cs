...
namespace APPLIB
{
  internal static class C3D
  {
    ...
    public static float FlipFloat(float inputF) => -inputF;
    ...
    public static float NanSafe(float val) => float.IsNaN(val) || (double) val + 0.0 == 0.0 ? 0.0f : Convert.ToSingle(val);
    ...
    public static Quaternion3D EulerAnglesToQuaternion(
      ...
      float roll)
    {
      double num1 = (double) C3D.NormalizeAngle(pitch);
      double num2 = (double) C3D.NormalizeAngle(yaw);
      ...
      {
        real = (float) (num6 * num4 * num5 + num9 * num7 * num8),
        i = (float) (num6 * num7 * num8 - num9 * num4 * num5),
        j = (float) (-num6 * num7 * num5 - num9 * num4 * num8),
        k = (float) (num6 * num4 * num8 - num9 * num7 * num5)
      };
    }
    ...
    private static float NormalizeAngle(float input) => (float) ((double) input * Math.PI / 360.0);

    public static Vector3D ToEulerAngles(Quaternion3D q) => C3D.Eul_FromQuat(q, 0, 1, 2, 0, C3D.EulerParity.Even, C3D.EulerRepeat.No, C3D.EulerFrame.S);
